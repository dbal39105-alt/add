...
# ============== BOT TOKEN ==============
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8777443945:AAGM_f-WtydhbzRDZ1bgMt1X6ATu_oushHs")  # <-- PUT YOUR TOKEN HERE
...
def create_session():
    ...
    session.mount('https://', requests.adapters.HTTPAdapter(
        pool_connections=100, pool_maxsize=300, max_retries=3, pool_block=False
    ))
    ...
    def get_captcha(self, user_id):
        ...
        try:
            response = self.session.post(
                ...
                json=captcha_data, timeout=(4, 10)
            )
    ...
    def send_aadhaar_otp(self, user_id, eid_number, captcha_value, captcha_txn_id, transaction_id):
        ...
        try:
            response = self.session.post(
                ...
                json=otp_request_data, timeout=(6, 15)
            )
            if response.status_code == 200:
                resp_json = response.json()
    ...
    def download_aadhaar_pdf(self, user_id, eid_number, otp, otp_txn_id, transaction_id, mask=False):
        ...
        try:
            response = self.session.post(
                ...
                json=download_data, timeout=(6, 20)
            )
            if response.status_code == 200:
                resp_json = response.json()
    ...
    def send_eid_otp(self, user_id, mobile, name, captcha_code, captcha_txn_id, transaction_id):
        ...
        try:
            response = self.session.post(
                ...
                json=request_data, timeout=(6, 15)
            )
            if response.status_code == 200:
                resp_json = response.json()
...
def _solve_anticaptcha(image_bytes):
    try:
        ...
        resp = requests.post(
            ...
            json={"clientKey": ANTI_CAPTCHA_KEY, "task": {"type": "ImageToTextTask", "body": img_b64}},
            timeout=6
        ).json()
        if resp.get("errorId") != 0:
            logger.warning(f"Anti-Captcha error: {resp.get('errorDescription')}")
            return None
        ...
        for _ in range(8):
            ...
            if result.get("status") == "ready":
                ...
                logger.info(f"Anti-Captcha solved: {sol}")
                return sol if sol else None
        logger.warning("Anti-Captcha timed out (falling back to user prompt)")
        return None
    except Exception as e:
...
def solve_captcha_auto(image_bytes):
    if ANTI_CAPTCHA_KEY:
        ...
        if result:
            return result
    return _solve_local(image_bytes)
...
def is_channel_member(user_id):
    """
    FIXED: Converts numerical chat ID correctly and safely verifies 
    membership endpoints through Telegram infrastructure constraints.
    """
    try:
        # Dynamic identifier casting ensures no integer-to-string format drop
        target_chat = int(CHANNEL_USERNAME) if CHANNEL_USERNAME.startswith("-100") or CHANNEL_USERNAME.isdigit() else CHANNEL_USERNAME
